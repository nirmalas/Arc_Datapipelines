...
def _derive_columns_from_att_matrix(
    ...
    pre_au_columns = list(base_columns[:au_idx - 1])
...
def _apply_round2(
    ...
    # ------------------------------------------------------------------ #
    # Vectorised replacement is much faster than applying a lambda per cell.
    df = df.replace(to_replace=r"^\s*(blank|nan|none|nat)?\s*$", value=None, regex=True)
...
def _build_lodm_lookup_indexes(lodm_df: pd.DataFrame) -> dict[str, Any]:
    ...
    for _, row in lodm_df.iterrows():
        ...
        if att_col and not _is_empty(row.get(att_col)):
...
def generate_single_mpdt(
    ...
    keep_indices = [
        ...
        if idx < attr_filter_start_idx or col in union_allowed_cols
    ]
