...
# Helpers
# ---------------------------------------------------------------------------
# --- AU+ helpers: att_matrix, LoDM maps, SmartForms lookup ---
...
def load_att_matrix_headers_from_template(template_path: Path, sheet_name: str = "att_matrix") -> tuple[list[str], list[str]]:
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        b = str(row[1] or "").strip()  # col B
        c = str(row[2] or "").strip()  # col C
        e = str(row[4] or "").strip()  # col E
        if not c:
            continue
        r1.append(c)
...
def build_final_au_headers(template_path: Path, lodm_df: pd.DataFrame, first15: int = 15) -> tuple[list[str], list[str]]:
    all_r1, all_r2 = load_att_matrix_headers_from_template(template_path, "att_matrix")
    perm_r1, perm_r2 = all_r1[:first15], all_r2[:first15]
    dyn_r1_all, dyn_r2_all = all_r1[first15:], all_r2[first15:]
...
def lookup_join2_value(join2_wide: pd.DataFrame, key_col: str, key_val: str, au_code: str, code_to_id: dict[str, str]) -> str:
    ...
    for cand in _column_name_candidates_for_code(au_code):
        ...
        if k2 in norm_map:
            ...
            return "" if pd.isna(v) else str(v).strip()
    # Try AttributeTypeId via LoDM map
...
def generate_single_mpdt(
    ...
    # --- BEGIN AU+ PATCH (insert after governance rows are written) ---

    # 0) Resolve paths/indices we already have in this function
    template_path = workspace / cfg.get("paths", {}).get("mpdt_template", "Input/C2-MPDT-Template-Mapping.xlsm")
    au_start_idx = column_index_from_string("AU") if column_index_from_string else 47  # your original uses 47 for AU
    count_header_norm = "count"

    # 1) Build AU headers from att_matrix + LoDM
    au_row1, au_row2 = build_final_au_headers(template_path, lodm_df, first15=15)

    # 2) Write AU headers in Row 1 and Row 2
    for off, (h1, h2) in enumerate(zip(au_row1, au_row2), start=0):
        ws.cell(row=1, column=au_start_idx + off, value=h1)
        ws.cell(row=2, column=au_start_idx + off, value=h2)

    # 3) Ensure SmartForms L3 is wide and find UAID_3 key column
    def _ensure_join2_wide(join2_df: pd.DataFrame) -> tuple[pd.DataFrame, str | None]:
        if join2_df is None or join2_df.empty:
            return pd.DataFrame(), None
        # Heuristic: if already wide (>50 cols), assume ready
        u3 = next((c for c in join2_df.columns if normalize_text(c) in {"uaid_3","uaid3","uaid","asset_id"}), None)
        if u3 and len(join2_df.columns) > 50:
            return join2_df.astype(str).fillna(""), u3
        ...
        if u3 and val_col and (attname_col or attid_col):
            df = join2_df[[u3, attname_col or attid_col, val_col]].copy()
            df.columns = [u3, "attr_key", "attr_val"]
            piv = df.pivot_table(index=u3, columns="attr_key", values="attr_val", aggfunc="first").reset_index()
            return piv.astype(str).replace("nan",""), u3
        return join2_df.astype(str).fillna(""), u3

    join2_wide, uaid3_col = _ensure_join2_wide(sf_l3)

    # 4) LoDM maps for AttributeTypeId fallback
    code_to_id, _class_to_codes = build_lodm_maps(lodm_df)

    # 5) Identify scope3 UAID_3 column name used in your data_rows
    #    Your original mpdt_df columns include 'UAID_3' already
    uaid3_col_in_rows = next((c for c in columns if normalize_text(c) in {"uaid_3","uaid3"}), "UAID_3")

    # 6) Identify the 'count' column index by scanning row-2 labels (keeps your template label)
    count_col_idx = None
    for c in range(1, ws.max_column + 1):
        v = ws.cell(row=2, column=c).value
        if v is not None and normalize_text(v) == count_header_norm:
            count_col_idx = c
            break

    # 7) Clear AU region for all data rows (avoid template leftovers)
    data_start = 3
    ...
    # 8) Fill AU+ values per row using UAID_3 and AU Row1 codes
    for row_offset, row_data in enumerate(data_rows):
        excel_row = data_start + row_offset
        uaid3_val = str(row_data.get(uaid3_col_in_rows, "")).strip()
        # Populate AU+ cells
        for off, code in enumerate(au_row1):
            val = lookup_join2_value(join2_wide, uaid3_col or uaid3_col_in_rows, uaid3_val, code, code_to_id)
            ws.cell(row=excel_row, column=au_start_idx + off, value=val)
        # Write count = LoDM attribute count + 10
        if count_col_idx is not None:
            class_code = str(row_data.get("HS2_Class") or row_data.get("AssetHierarchyCategory") or row_data.get("ClassCode") or "").strip()
            ws.cell(row=excel_row, column=count_col_idx, value=lodm_attr_count_for_class(lodm_df, class_code) + 10)

    # 9) Apply checkerboard using your existing applicability builder + apply_applicability
    #    Build a minimal row1_codes map for AU+ we just set so the builder can do "exact AttTypeName matching" [1.3]
    row1_codes = row1_codes or {}
    # Map Row-2 labels under AU to Row-1 codes
    for off, (h1, h2) in enumerate(zip(au_row1, au_row2), start=0):
        row1_codes[str(h2).strip()] = str(h1).strip()

    # Recompute allowed sets using your existing helper (_build_row_allowed_columns) [1.3, 1.4]
    aw_start_idx = column_index_from_string("AW") if column_index_from_string else 49  # original uses AW as applicability start
    row_allowed_cols, union_allowed_cols = _build_row_allowed_columns(
        mpdt_df,  # the DataFrame you just wrote to worksheet in your original flow
        columns,  # original full header list (Row-2 labels)
        lodm_df,
        aw_start_idx,
        logger,
        row1_codes=row1_codes,   # critical: ensures exact matching by Row-1 short code
    )

    # Snapshot template row fills and apply applicability on the final coordinates [1.2]
    template_fills = _snapshot_row_fills(ws, data_start, ws.max_column)
    apply_applicability(
        ws,
        data_rows,
        columns,            # original columns (Row-2 headers)
        row_allowed_cols,
